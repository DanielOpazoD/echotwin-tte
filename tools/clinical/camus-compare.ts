...
interface Quartiles { median: number; p25: number; p75: number; n: number }
const quantiles = (v: number[]): Quartiles => {
  ...
  return { median: at(0.5), p25: at(0.25), p75: at(0.75), n: s.length };
};
  ...
  for (const [ch] of VIEWS)
    for (const phase of PHASES) {
      ...
      for (const [key, , get] of METRICS) {
        const q = quantiles(clin.map(get));
        body += `    ${key}: { median: ${f(q.median)}, p25: ${f(q.p25)}, p75: ${f(q.p75)}, n: ${q.n} },\n`;
      }
      body += '  },\n';
    }
  const module =
