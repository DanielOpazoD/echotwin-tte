 ...
 *      [--sector-out src/clinical/reference-values/camusSectorStats.ts]
...
const sectorOut = arg('--sector-out');
...
// the whole sector of the optimal-window images, without labels (decision 146), keyed by view and phase
const sector = new Map<string, SectorStats[]>();
  ...
  for (const ch of ['4CH', '2CH'] as const) {
    ...
    for (const phase of ['ED', 'ES'] as const) {
      ...
      if (quality === 'Good') {
        ...
        if (!sector.has(key)) sector.set(key, []);
        sector.get(key)!.push(sectorStats(oriented));
      }
    }
  }
...
{
  process.stdout.write(
    '\n=== whole sector, no labels: CAMUS Good vs simulator (normal-excellent-window, default console; decision 146)\n',
  );
  for (const [ch, view] of VIEWS)
    for (const phase of PHASES) {
      const clin = sector.get(`${ch}-${phase}`) ?? [];
      const sim = sectorStats(presentApicalFrame(renders, 'normal-excellent-window', view, phase));
      for (const k of SECTOR_METRICS) {
        const q = quantiles(clin.map((g) => g[k]));
        if (!Number.isFinite(q.median)) continue;
        const v = sim[k];
        const flag = v < q.p25 ? 'below' : v > q.p75 ? 'above' : 'within';
        process.stdout.write(
          `  ${ch} ${phase} ${k.padEnd(18)} ${q.median.toFixed(3).padStart(8)} [${q.p25.toFixed(3)}–${q.p75.toFixed(3)}]  sim ${v.toFixed(3).padStart(8)}  ${flag}\n`,
        );
      }
    }
}
if (sectorOut) {
  const f = (x: number): string =>
    Number.isFinite(x) ? String(Math.round(x * 1000) / 1000) : 'NaN';
  let body = '';
  for (const [ch] of VIEWS)
    for (const phase of PHASES) {
      const clin = sector.get(`${ch}-${phase}`) ?? [];
  ...
  const module =
    ...
    `export const CAMUS_GOOD_SECTOR: Record<'4CH-ED' | '4CH-ES' | '2CH-ED' | '2CH-ES', Partial<Record<SectorMetric, Quartiles>>> = {\n${body}};\n`;
  writeFileSync(sectorOut, module);
  process.stdout.write(`sector reference written to ${sectorOut}\n`);
}
