...
- `CAMUS_DIR=~/datos/CAMUS/database_nifti npx tsx tools/clinical/blind-test.ts --out ~/datos/CAMUS/blind [--n 12]` — prueba ciega (decisión 144): página con mitad imágenes CAMUS Good y mitad simuladas a la misma escala, orden aleatorio y clave aparte; la salida debe quedar fuera del repositorio (lleva píxeles clínicos). Es la medida final de «que parezca eco»; la primera mirada (22-09) delata al simulador por la textura (speckle fino y uniforme) antes que por la anatomía.
