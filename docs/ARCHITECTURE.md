...
| Casos                   | `src/cases`                                        | `CaseDefinitionSchema` (Zod) y 12 casos (`docs/CLINICAL_SCOPE.md`)                                                                                                                              |
| Anatomía                | `src/simulator/anatomy`                            | corazón paramétrico por SDF, tórax, tabla de tejidos                                                                                                                                            |
| Ciclo cardíaco          | `src/simulator/cardiac-cycle`                      | timings, tablas de latido, reloj RR, ECG sintético                                                                                                                                              |
| Hemodinámica            | `src/simulator/hemodynamics`                       | `StructuredEchoTruth` a partir de las tablas                                                                                                                                                    |
| Sonda / ventanas        | `src/simulator/probe`, `src/simulator/windows`     | `ProbeControl → ProbePose → BeamFrame`; vistas canónicas                                                                                                                                        |
| Renderer                | `src/simulator/renderer`                           | descriptor de modalidades (`modality.ts`, decisión 127), interfaz `RendererBackend`, trazador procedimental, atlas, consola, scan conversion, frame rate                                        |
| Doppler                 | `src/simulator/doppler`                            | campo de flujo, color, espectral, audio                                                                                                                                                         |
| Mediciones              | `src/simulator/measurements`                       | herramientas de medición (`simpson.ts`, `vti.ts`), geometría de calibres, protocolo de medidas y tipos del resultado de técnica                                                                 |
| Reconocimiento de vista | `src/simulator/view-recognition`                   | score 0–100 y hints deterministas                                                                                                                                                               |
| Núcleo del simulador    | `src/simulator/core`                               | `SimulatorCore` (headless), `protocol.ts`, `client.ts`                                                                                                                                          |
| Clínica                 | `src/clinical`                                     | fórmulas puras, referencias versionadas, rangos normales, valores de referencia, lector NIfTI y estadísticas de región (capa hoja: cualquier capa puede leerla, ella no importa nada del motor) |
| Educación               | `src/education`                                    | técnica de medición, currículo, causas, impresión, informe educativo (`report.ts`) y `scoring/` (adquisición, mediciones, resumen de examen)                                                    |
| Aplicación              | `src/app`, `src/ui`, `src/workers`, `src/main.tsx` | React 19, zustand, `frameBus`, three.js (torso), tutorial de 8 pasos (`Tutorial.tsx`), exportación PNG con marca de agua (`exportImage.ts`)                                                     |

Qué importa cada capa lo dice la tabla siguiente, generada desde los imports reales de `src/` (sin pruebas; decisión 177) con `npx tsx tools/docs/layer-graph.ts`. `docsConsistency.test.ts` falla si no coincide con el código, así que una arista nueva entre capas aparece en la revisión del cambio que la crea. Los paquetes (zod, React, three.js) no figuran.

<!-- generado por tools/docs/layer-graph.ts: no editar a mano -->
| Capa | Importa de |
|---|---|
| `app` | `cases`, `education`, `simulator/anatomy`, `simulator/cardiac-cycle`, `simulator/core`, `simulator/doppler`, `simulator/hemodynamics`, `simulator/measurements`, `simulator/probe`, `simulator/renderer`, `simulator/windows`, `ui` |
| `cases` | — |
| `clinical` | — |
| `core` | — |
...
| `workers` | `cases`, `simulator/anatomy`, `simulator/cardiac-cycle`, `simulator/core`, `simulator/windows` |
<!-- /generado -->
...
Por cada step(dt) en SimulatorCore:
  ...
  o backend.render(…) + applyConsole     → PolarFrame con amplitud lineal → intensidades 0..255 (CPU; artefacto espejo)
...
3. **Propagación**: transmisión de ida y vuelta acumulada a lo largo de la línea (sombras), entrada en pulmón con reverberación, clutter de campo cercano fijo a la sonda. Desde la decisión 144 el incremento de atenuación que paga una línea a cada profundidad es la media de los incrementos de las líneas dentro de la semianchura del haz allí (`beamHalfWidthCm`: la apertura afilándose hacia el foco), porque la energía que llega a una muestra cruzó, a cada profundidad del camino, la anchura del haz y no una línea: en CPU `beamMarch` lo hace tras clasificar todas las líneas (sumas prefijas por muestra), en GPU la pasada P antes de la marcha B. El eco de cada muestra lleva además la sensibilidad en eje del haz a esa profundidad (`focusingGain`: menor junto a la cara, donde el haz es tan ancho como la apertura, máxima en el foco), que la consola devuelve sólo más allá del foco.
4. **Señal compleja**: σ × fasor de dispersores anclado al tejido + eco especular + granos coherentes del parénquima (decisión 145: la media ¼ ½ ¼ de tres retículos independientes a la frecuencia de grano del tejido —el corte integra dos o tres celdas de grano de través—, con la celda de través del grosor de corte como el fasor, umbral y ganancia sobre σ), por la transmisión del haz. Desde la decisión 145 un cuadro forma `COMPOUND_LOOKS` señales (dos), una por mirada: el mismo eco coherente y un fasor de dispersores distinto (retículo desplazado), también para el clutter y la reverberación pleural. La línea de modo M coloca sus granos por tramo de estructura tras la marcha, como sus dispersores.
5. **PSF y envolvente** (`renderer/acoustic/psf.ts`): pulso axial (× `PSF_AXIAL_SCALE`, el filtro armónico de banda estrecha) y haz lateral de ida y vuelta según profundidad, foco, frecuencia y armónicos, con núcleos de energía unidad; después, módulo de la señal de cada mirada y media de las envolventes (compounding). El resultado es la amplitud lineal del `PolarFrame`; el ruido del receptor que añade la consola va dividido por √miradas.

La consola trabaja sobre esa envolvente (ganancia, TGC, ruido electrónico Rayleigh, compresión, realce, persistencia y mapa de grises) y ya no aplica resolución. En la GPU las etapas 1–2 son la pasada A (cuatro destinos: σ/atenuación, identificadores, especular/fasor/coeficiente de grano, fasor de la segunda mirada), la 3 la pasada P (incremento de atenuación promediado sobre el haz) y la marcha B, que también forma la 4, y la 5 las pasadas C (axial, las dos miradas) y D (lateral, envolvente de cada mirada y su media), que leen la misma tabla de núcleos Float32 que la CPU desde una textura. `acoustic/acoustics.ts` define las constantes compartidas y las exporta a GLSL como `#define`.

### Espejo GLSL del clasificador

`gpu/glslHeart.ts` es un port a mano de `anatomy/classify.ts` y sus módulos, función por función. Desde la decisión 126 el clasificador CPU es un orquestador de siete bloques en `anatomy/classify/` (`root`, `valves`, `leftVentricle`, `aorticRoot`, `atria`, `rightVentricle`, `pericardium`) que comparten un contexto (`classify/context.ts`) en el mismo orden de prioridad que el sombreador; los módulos de primitivas (`valveSkirt`, `lvWall`, `rv`, `mitralValve`, `aorticValve`) siguen debajo de ambos. Los parámetros de pose llegan por la textura de parámetros (`paramLayout.ts`, `P(i)` con `#define` de desplazamientos) y los enumerados de tejido y estructura por `glslCommon.ts`; ambos son de fuente única. **Convención**: toda constante numérica que compartan el clasificador CPU y su port debe ser un `export const` en el módulo TS de anatomía e interpolarse en la cabecera de `GLSL_HEART` (`const float NOMBRE = ${f(NOMBRE)};`), nunca un literal repetido a ambos lados. `glslHeart.test.ts` comprueba en Node que cada identificador en mayúsculas que usa el shader está declarado, que cada constante interpolada se usa y que su valor es un literal GLSL bien formado; la equivalencia numérica CPU↔GPU sigue siendo cosa de `e2e/gpu-equivalence.spec.ts` en el navegador, que desde la decisión 124 acota también el desacuerdo por estructura. Las funciones escalares puras compartidas por los dos lados (`saddleOffset`, `ellipseFactor`, `axialWallFactor`, `rvAxialTaper`, `septalShiftAt`, `myoHelixGain`, `pleuralReverberation`, `sliceHalfWidthCm`; `myoAnisoGain`, la dirección circunferencial única de la decisión 123, se retiró sin uso tras la hélice de la decisión 144) no se espejan: `tools/glsl/ts2glsl.ts` las transpila desde su TypeScript a `gpu/glslGenerated.ts` (`npm run glsl:gen`, decisión 125) y `glslGenerated.test.ts` falla si el archivo está desactualizado.
...
### Torso arriba, mapa del plano abajo (decisiones 137 y 142)

`TorsoView.tsx` monta el lienzo WebGL del torso (`.torso-3d`: sonda, plano, cámara orbital y todos los gestos) y, con la capa «Corte de frente» activa, `CutMapView.tsx` debajo (`.cut-map`): un lienzo 2D que pinta, en cada cuadro, el mapa de estructuras que el propio cuadro trae del trazador (`SimOutput.structure`, línea × muestra, en CPU y en GPU) convertido a sector con el mismo `computeSectorMapping` de la imagen (profundidad, zoom e inversión izquierda/derecha incluidos). Es el plano de la imagen por construcción: no hay segunda cámara ni segundo corte del modelo. `cutMap.ts` (puro, probado) construye la tabla de la muestra más cercana por píxel, pinta con la paleta de `structurePalette.ts` (compartida con el mapa offline `slice-map.ts`), acumula las regiones y coloca las etiquetas sobre un píxel de su región, sin solapes, con histéresis y a 400 ms; el componente añade la regla de profundidad y el nombre completo de la estructura bajo el ratón. El mapa no recibe gestos; las capas `navSplit` y `navLabels` lo encienden y etiquetan.
