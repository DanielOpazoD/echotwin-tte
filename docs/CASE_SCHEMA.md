...
| `anatomy.wallMotion[]` | `segment` 1–17, `amplitude` −0,5–1,2, `delayPhase`, `score` 1–4 | `delayPhase` se ignora. `amplitude` es la mecánica que dibuja el simulador (contracción regional), no una puntuación; `score` es la lectura que el caso declara para su verdad sintética (1 normal, 2 hipocinético, 3 acinético, 4 discinético), obligatoria en los segmentos 1–16 cuya amplitud no es 1 y prohibida en el 17, que no se puntúa (decisión 152). La verdad de terreno toma el WMSI de estas puntuaciones, nunca de la amplitud. El casquete (17) está más allá de la cavidad: su amplitud sólo cambia su engrosamiento, no mueve la cavidad; la motilidad del ápex se declara en 13–16, que lo contienen en el modelo de 16. |
