...
`t_línea = 2·profundidad/1540 + 20 µs`; `fps = 1/(líneas·t_línea·zonas_focales + líneas_color·8·t_línea)`, tope 90 Hz. Las líneas dependen del nivel de calidad (`polarSpecFor`); la calidad automática, la de arranque, es la alta cuando la GPU forma la imagen (decisión 154), así que el FR por defecto es 27,3 Hz. Enseña la relación profundidad / sector / densidad / caja de color; no reproduce ningún equipo. El worker usa este valor para su cadencia real (acotada a 12–50 ms por paso) y la UI lo muestra como «FR».
