...
| `tricuspid-inflow` | De −2,2 a 5 cm respecto al anillo tricúspide | Tabla tricuspídea propia, área 1,35·A_mv, con la onda E 20 ms antes que la mitral porque la tricúspide abre antes (decisión 162), decaimiento `1/(1 + (z − 1,2)/2,2)` y ensanchamiento sin tope | `turbulence` |
| `rvot` | Cápsula entre `rvotA` y `rvotB`, radio 1,05 cm | `pulmonaryFlowMlps/(π·1,1²)` durante la eyección del VD, que empieza 15 ms antes que la aórtica y termina 25 ms después (decisión 162), con el tiempo de aceleración de la presión pulmonar (decisión 105); perfil `1 − (ρ/1,05)⁶`; desciende con la base (decisión 111) | `turbulence` |
