...
describe('bone attenuation is integrated over distance', () => {
  ...
  const scene: Scene = { heart, heartPose: computeHeartPose(heart, cycleStateAt(tables, 0.1)), thorax, physics: { frequencyMHz: 2.5, harmonics: true, clutterLevel: 0.1, windowAttenuation: 0.1, seed: 3 } };
  ...
  const shadowDb = (offsetV: number, tier: 'low' | 'medium' | 'high'): { db: number; boneLineShare: number; pathMm: number } => {
    ...
    for (let li = 0; li < spec.lines; li++) {
      ...
      lines++;
      path += inBone * dr;
      sum += 20 * Math.log10(Math.max(1e-6, f.transmission[li * spec.samples + Math.min(spec.samples - 1, last + Math.round(1 / dr))]!));
    }
    return { db: sum / Math.max(1, lines), boneLineShare: lines / spec.lines, pathMm: (10 * path) / Math.max(1, lines) };
  };
  ...
  it('the same rib shadows alike in the low, medium and high tiers', { timeout: 120_000 }, () => {
    ...
    for (const offsetV of [1.4, -1.4]) {
      ...
      expect(Math.max(...tiers.map((t) => t.pathMm)) - Math.min(...tiers.map((t) => t.pathMm))).toBeLessThan(1);
      expect(Math.max(...dbs) - Math.min(...dbs), `probe v${offsetV}: ${dbs.map((d) => d.toFixed(1)).join(' / ')} dB`).toBeLessThan(3);
    }
  });
});
