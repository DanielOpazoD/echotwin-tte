...
describe('point spread function and envelope', () => {
  ...
  it('the beam-width artifact widens the lateral beam away from the focus, not at it', () => {
    expect(lateralFwhmMm(14, 6, 2.5, true, 1)).toBeGreaterThan(
      lateralFwhmMm(14, 6, 2.5, true, 0) * 1.5,
    );
  ...
  it('the tissue-anchored scatterer phasor has zero mean, unit power and uncorrelated parts', () => {
    ...
    for (let k = 0; k < N; k++) {
      ...
      const zr =
        (latticeNoise3(x, y, z, latA) +
          latticeNoise3(
            x * SCATTER_FREQ_RATIO + 37.3,
            y * SCATTER_FREQ_RATIO + 11.9,
            z * SCATTER_FREQ_RATIO + 23.7,
            latB,
          ) -
          1) *
        PHASOR_NORM;
      const zi =
        (latticeNoise3(x + 71.1, y + 53.5, z + 5.3, latC) +
          latticeNoise3(
            x * SCATTER_FREQ_RATIO + 17.9,
            y * SCATTER_FREQ_RATIO + 91.1,
            z * SCATTER_FREQ_RATIO + 43.3,
            latA,
          ) -
          1) *
        PHASOR_NORM;
  ...
  describe('M-mode line kernels keep the frame levels at a finer sampling', () => {
    ...
    const speckleMean = (
      ...
      for (let t = 0; t < rays; t++) {
        ...
        for (let i = 0; i < samples; i++) {
          ...
          re[i] =
            (latticeNoise3(x, y, z, latA) +
              latticeNoise3(x * R + 37.3, y * R + 11.9, z * R + 23.7, latB) -
              1) *
            PHASOR_NORM *
            scale;
          im[i] =
            (latticeNoise3(x + 71.1, y + 53.5, z + 5.3, latC) +
              latticeNoise3(x * R + 17.9, y * R + 91.1, z * R + 43.3, latA) -
              1) *
            PHASOR_NORM *
            scale;
        }
    ...
    const interfacePeak = (
      ...
      for (let j = 0; j < positions; j++) {
        ...
        for (let i = 0; i < samples; i++)
          if (Math.abs(r0 - (i + 0.5) * dr) < Math.max(1, SPECULAR_WINDOW_MIN) * dr)
            re[i] = amplitude;
        formEnvelopeLine(
          re,
          im,
    ...
    it('the filtered power of the scatterer lattice follows its autocorrelation: 1 at the frame spacing, +4.25 dB at a quarter of it', () => {
      ...
      const db =
        10 * Math.log10(filteredScattererPower(line.axial, spec.depthCm / (spec.samples * 4)));
      ...
      const measured =
        20 *
        Math.log10(
          speckleMean(spec.samples * 4, line, 1, 40, 'frame') /
            speckleMean(spec.samples, frame, 1, 160, 'frame'),
        );
    ...
    it('speckle and interface echoes of a line four times finer have the levels of a frame line', () => {
      ...
      for (const samples of [spec.samples * 2, 800, spec.samples * 4]) {
        ...
        const speckleDb =
          20 * Math.log10(speckleMean(samples, line, line.incoherent, rays, 'line') / frameSpeckle);
        ...
        const fixedDb =
          20 *
          Math.log10(
            speckleMean(samples, line, line.incoherent, rays, 'line', 40.5) / frameSpeckle,
          );
        expect(Math.abs(fixedDb)).toBeLessThan(0.5);
        const interfaceDb =
          20 *
          Math.log10(
            interfacePeak(samples, line, line.specular) / interfacePeak(spec.samples, frame, 1),
          );
