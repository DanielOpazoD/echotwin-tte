...
import { applyConsole, consoleCompensation, createConsoleState, NOISE_FLOOR, NOISE_RMS, receiverNoise, REF_DB } from './consolePipeline';
import { buildNoiseKernels, LATERAL_TAPS, MAX_LATERAL_RADIUS, type PsfKernels } from '../acoustic/psf';
 ...
 * resolution.
 */
const settings = { ...DEFAULT_ACQUISITION, grayMap: 'linear' as const, dynamicRangeDb: 90, gainDb: 0, edgeEnhance: 0, persistence: 0, tgcDb: [0, 0, 0, 0, 0, 0, 0, 0] };

/** Lag-1 correlation of the noise power |n|² along the beam (dl 0, ds 1) or across lines (dl 1, ds 0), mid-depth band. */
function powerCorrelation(spec: PolarFrameSpec, k: PsfKernels, dl: number, ds: number, frames = 3): number {
  const { lines: L, samples: N } = spec;
  const re = new Float32Array(L * N), im = new Float32Array(L * N), tr = new Float32Array(L * N), ti = new Float32Array(L * N);
  let c = 0;
  for (let f = 0; f < frames; f++) {
    ...
    c += (sxy / n - (sx / n) * (sy / n)) / Math.sqrt((sxx / n - (sx / n) ** 2) * (syy / n - (sy / n) ** 2)) / frames;
  }
  return c;
}
  ...
  it('adds to the echo before detection: a steady echo reads the Rician second moment, and noise alone its floor', () => {
    ...
    for (let li = 0; li < L; li++)
      for (let si = Math.floor(0.3 * N); si < Math.floor(0.7 * N); si++) {
        const db = (display[li * N + si]! / 255) * settings.dynamicRangeDb - settings.dynamicRangeDb + REF_DB;
    ...
    const re = new Float32Array(L * N), im = new Float32Array(L * N);
