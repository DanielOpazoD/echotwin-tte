    ...
    for (const tier of ['low', 'medium', 'high'] as const) {
      ...
      for (let l = 0; l < spec.lines; l++)
        for (let s = 0; s < spec.samples; s++)
          polar[l * spec.samples + s] = ((l * 7 + s * 3) % 256) ^ (s === 100 ? 255 : 0);
      for (const [w, h, invert] of [
        [512, 440, false],
        [890, 680, true],
      ] as const) {
        ...
        for (let i = 0; i < fast.length; i++)
          maxDiff = Math.max(maxDiff, Math.abs((fast[i] ?? 0) - (ref[i] ?? 0)));
        expect(maxDiff, `${tier} ${w}x${h}`).toBeLessThanOrEqual(1);
        const corner = 0; // top-left pixel is outside the sector
        expect([fast[corner], fast[corner + 1], fast[corner + 2], fast[corner + 3]]).toEqual([
          0, 0, 0, 255,
        ]);
      }
    }
        ...
        for (let p = 0; p < w * h; p++) {
          ...
          if (c2 & LUT_TEXEL_INSIDE) {
            ...
            const dl = (c2 >> 10) & 1,
              ds = (c3 >> 10) & 1;
            g =
              ((at(s0, l0) * (1024 - lt) + at(s0, l0 + dl) * lt) * (1024 - st) +
                (at(s0 + ds, l0) * (1024 - lt) + at(s0 + ds, l0 + dl) * lt) * st) >>>
              20;
