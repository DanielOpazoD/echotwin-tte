...
import { allocPolarFrame, DEFAULT_ACQUISITION, polarSpecFor, type PolarFrame, type PolarFrameSpec, type RendererBackend, type Scene } from '../types';
...
function fillCine(atlas: AtlasRenderer, beam: BeamFrame, out: PolarFrame): number {
  ...
  for (let k = 0; k < 6 * ATLAS_PHASES && atlas.stats()['cineFill'] !== FULL; k++) {
    ...
    atlas.render(scene(ph), beam, spec, ph, out, { stationary: true, budgetMs: 0, sceneAtPhase: scene });
    frames++;
  }
  return frames;
}
...
describe('atlas render cache: the image is always the image of the current pose', () => {
  it('with a source that fits the frame budget it renders the exact pose and phase and keeps no cine', { timeout: 60_000 }, () => {
    const src = new CountingSource();
    const atlas = new AtlasRenderer(src, 1);
    const beam = beamOf(plax);
    const out = allocPolarFrame(spec);
    for (let i = 0; i < 40; i++) atlas.render(scene(0.27), beam, spec, 0.27, out, { stationary: true, budgetMs: 1e9 });
  ...
  it('with a slow source at rest it fills a cine within about one beat, one render per empty slot, then serves it', { timeout: 60_000 }, () => {
    const src = new CountingSource();
    const atlas = new AtlasRenderer(src, 1);
    const beam = beamOf(plax);
    const out = allocPolarFrame(spec);
    ...
    atlas.render(scene(0.27), beam, spec, 0.27, out, { stationary: true, budgetMs: 0, sceneAtPhase: scene });
    ...
    expect(meanAbsDiff(out.amplitude, directRender(beam, 9 / ATLAS_PHASES).amplitude)).toBeLessThan(0.002);
  });

  it('any movement beyond 0.1 mm or 0.08° shows the new pose: no dead zone and no blending with the cine', { timeout: 120_000 }, () => {
    ...
    for (const [label, ctrl] of moves) {
      ...
      for (const stationary of [false, true]) {
        atlas.render(scene(0.25), beam, spec, 0.25, out, { stationary, budgetMs: 0, sceneAtPhase: scene });
  ...
  it('sweeping PLAX → PSAX past a complete cine degrades continuously (no frame jumps)', { timeout: 120_000 }, () => {
    const atlas = new AtlasRenderer(new CountingSource(), 1);
    const out = allocPolarFrame(spec);
