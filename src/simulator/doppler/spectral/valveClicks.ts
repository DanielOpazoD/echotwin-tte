...
function valveDistance(px: number, py: number, pz: number, cx: number, cy: number, cz: number, ax: number, ay: number, az: number, radius: number): number {
  const dx = px - cx,
    dy = py - cy,
    dz = pz - cz;
 ...
 */
export function valveClickWeight(heart: HeartModel, hp: HeartPose, tables: BeatTables, timeInBeatS: number, points: ArrayLike<number>): number {
  const A = heartAnchors(heart);
  const events = valveEventTimes(tables);
  const rr = tables.rrS;
  const valves: { c: [number, number, number]; a: [number, number, number]; r: number; times: [number, number] }[] = [
    { c: [A.mvCenter.x, A.mvCenter.y, hp.zAnn], a: [0, 0, 1], r: A.mvR, times: events.mitral },
    { c: [A.avCenter.x, A.avCenter.y, A.avCenter.z + hp.zAnn * ROOT_EXCURSION], a: [A.avAxis.x, A.avAxis.y, A.avAxis.z], r: A.avR, times: events.aortic },
    { c: [A.tvCenter.x, A.tvCenter.y, A.tvCenter.z + hp.tvZ], a: [0, 0, 1], r: A.tvR, times: events.tricuspid },
    { c: [A.rvotB.x, A.rvotB.y, A.rvotB.z], a: [A.paDir.x, A.paDir.y, A.paDir.z], r: A.pvR, times: events.pulmonary },
  ];
  let weight = 0;
  for (const v of valves) {
    const inTime = Math.max(clickTime(timeInBeatS, v.times[0], rr), clickTime(timeInBeatS, v.times[1], rr));
    if (inTime < 1e-3) continue;
    let d = Infinity;
    for (let i = 0; i + 2 < points.length; i += 3) d = Math.min(d, valveDistance(points[i]!, points[i + 1]!, points[i + 2]!, v.c[0], v.c[1], v.c[2], v.a[0], v.a[1], v.a[2], v.r));
