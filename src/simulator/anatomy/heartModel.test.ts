...
describe('heart model geometry', () => {
  ...
  it('LV cavity volume at ED ≈ EDV and at ES ≈ ESV (Monte Carlo, ±12%)', { timeout: 60_000 }, () => {
  ...
  it('mitral valve open in early diastole, closed in systole; aortic the opposite', () => {
    ...
    const midEj = computeHeartPose(model, cycleStateAt(tables, (tables.timings.ejectionStartS + 0.1) / tables.rrS));
    expect(midEj.avOpenAngle).toBeGreaterThan(0.6);
    expect(esPose.avOpenAngle).toBeLessThan(0.4); // closing at end-ejection
    const early = computeHeartPose(model, cycleStateAt(tables, (tables.timings.mitralOpenS + tables.timings.eAccelS) / tables.rrS));
