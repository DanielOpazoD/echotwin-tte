 ...
 */
const gpu = vi.hoisted(() => ({ lost: false, loseDuringNext: false, displays: 0, presents: 0, restores: 0, lastColor: false, colourVersions: [] as number[] }));
...
vi.mock('@/simulator/renderer/gpu/webgl2Renderer', async () => {
  ...
  class StandInGpu {
    ...
    render(scene: Scene, beam: BeamFrame, spec: PolarFrameSpec, phase: number, out: PolarFrame): void {
      this.cpu.render(scene, beam, spec, phase, out);
    }
    renderDisplay(scene: Scene, beam: BeamFrame, spec: PolarFrameSpec, phase: number, out: PolarFrame, _hints: unknown, con: DisplayConsole, display: Uint8ClampedArray): boolean {
...
describe('GPU image chain routing in the core (stand-in renderer)', () => {
  beforeEach(() => Object.assign(gpu, { lost: false, loseDuringNext: false, displays: 0, presents: 0, restores: 0, lastColor: false, colourVersions: [] }));

  it('presents live 2D and colour frames from the GPU and keeps the CPU composite for strips, cine review and console artifacts', { timeout: 60_000 }, () => {
    ...
    expect(gpu.colourVersions.every((v) => v === -1), '2D frames carry no colour field').toBe(true);
    ...
    expect(Math.max(...versions.map((v, i) => (i ? v - versions[i - 1]! : 0))), 'one step per update').toBe(1);
    ...
    core.setInput(input({ artifactOverrides: { sideLobe: 0, mirror: 0.5, beamWidth: 0, clutter: 0 } }));
  ...
  it('forms the frame again with the CPU tracer when the context is lost while forming it', { timeout: 60_000 }, () => {
    const core = new SimulatorCore(loadCaseById('normal-excellent-window'), input());
    stepFrames(core, 2);
    ...
    expect(sum / (px.length / 4), 'the re-formed frame is an image, not the stale or empty GPU read').toBeGreaterThan(1);
