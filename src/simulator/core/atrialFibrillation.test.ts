...
describe('atrial fibrillation beat by beat (decision 107)', () => {
  ...
  const beats = (view: 'a4c' | 'a5c', gate: Vec3, seconds: number): Beat[] => {
    ...
    for (let i = 1; i < cols.length; i++) {
      ...
      if (start >= 0)
        out.push({
          rr: cols[i]!.t - cols[start]!.t,
          cols: cols.slice(start, i).map((q) => ({ t: q.t - cols[start]!.t, v: q.v })),
        });
      start = i;
    }
    return out;
  ...
  it(
    'through the core: every beat whose E wave fits reads the case deceleration time, whatever its interval',
    { timeout: 480_000 },
    () => {
      ...
      mitral.forEach((b, i) => {
        ...
        const mvo =
          0.06 +
          ejectionTimeS(60 / mitral[i - 1]!.rr, c.physiology.contractility) +
          c.physiology.ivrtMs / 1000;
        if (
          mvo +
            Math.min(0.1, c.physiology.decelerationTimeMs / 2000) +
            c.physiology.decelerationTimeMs / 1000 +
            0.03 >
          b.rr
        )
          return;
  ...
  it(
    'through the core: the outflow VTI follows the filling of the interval before, not the beat’s own interval',
    { timeout: 480_000 },
    () => {
      const lvot = beats(
        'a5c',
        v3(
          A.avCenter.x - 0.6 * A.avAxis.x,
          A.avCenter.y - 0.6 * A.avAxis.y,
          A.avCenter.z - 0.6 * A.avAxis.z,
        ),
        16,
      );
      const full: number[] = [];
      ...
      lvot.forEach((b, i) => {
        if (i === 0) return;
        let vti = 0;
        for (let k = 1; k < b.cols.length; k++)
          if (b.cols[k]!.t < 0.45 && b.cols[k]!.v < -0.1)
            vti -= b.cols[k]!.v * (b.cols[k]!.t - b.cols[k - 1]!.t) * 100;
        report.push(
          `${(lvot[i - 1]!.rr * 1000).toFixed(0)}→${(b.rr * 1000).toFixed(0)}:${vti.toFixed(1)}`,
        );
