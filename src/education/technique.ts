...
const BLOOD_STRUCTURES = new Set<number>([
  Structure.LvCavity,
  Structure.RvCavity,
  Structure.LaCavity,
  Structure.RaCavity,
...
function phaseWindow(
  ...
  switch (req) {
    ...
    case 'mid-systole':
      return {
        lo: m.ejectionStart + 0.15 * (m.ejectionEnd - m.ejectionStart),
        hi: m.ejectionStart + 0.7 * (m.ejectionEnd - m.ejectionStart),
        label: 'mesosístole (válvula aórtica abierta)',
      };
    case 'systole':
    ...
    case 'late-diastole':
      return m.hasAWave
        ? { lo: m.aStart - 0.02, hi: m.aEnd + 0.03, label: 'diástole tardía (onda A)' }
        : null;
    default:
      return null;
  }
...
export function evaluateTechnique(spec: MeasurementSpec, ctx: MeasurementContext): TechniqueResult {
  ...
  const add = (code: string, level: FindingLevel, message: string) =>
    findings.push({ code, level, message });

  // modality
  const modalityFamily = ctx.modality === 'color' ? '2d' : ctx.modality;
  if (!spec.modalities.includes(modalityFamily as MeasurementSpec['modalities'][number]))
    add(
      'modality',
      'invalid',
  ...
  if (
    ctx.phaseMarks &&
    spec.phase !== 'any' &&
    (spec.tool === 'caliper' || spec.tool === 'simpson')
  ) {
    ...
    if (w) {
      if (inWindow(ctx.phase, w.lo, w.hi)) add('phase', 'ok', `Cuadro en ${w.label}.`);
        ...
        add(
          'phase',
          dist > 0.12 ? 'invalid' : 'warn',
          `Cuadro fuera de ${w.label} (fase ${(p * 100).toFixed(0)} % del ciclo): usa el cine para elegir el cuadro correcto.`,
        );
      }
    }
  }
  ...
  if (spec.placement) {
    ...
    if (
      ...
      if (!g) add('placement', 'warn', 'Sin información del volumen de muestra.');
        ...
        if (pl.structures.includes(g.structure))
          add('placement', 'ok', `Volumen de muestra en ${pl.label}.`);
        else
          add(
            'placement',
            'invalid',
            `Volumen de muestra fuera de posición (${structureName(g.structure)}): debe estar en ${pl.label}.`,
          );
      } else {
      ...
      spec.tool === 'caliper' &&
      ctx.segmentStructures &&
      ctx.segmentStructures.length > 2
    ) {
      ...
      if (pl.segment === 'cavity') {
        ...
        if (inTarget < 0.5)
          add(
            'placement',
            'invalid',
            `El caliper no está sobre ${pl.label} (${(inTarget * 100).toFixed(0)} % del segmento en la estructura esperada).`,
          );
        else if (blood < 0.8)
          add(
            'placement',
            'warn',
            `El caliper incluye pared (${((1 - blood) * 100).toFixed(0)} % del segmento fuera de la cavidad): mide de borde interno a borde interno.`,
          );
        else add('placement', 'ok', `Caliper sobre ${pl.label}.`);
        const ends = ctx.segmentEndsOutside;
        if (ends && blood >= 0.8) {
          const endsOnTissue = ends.filter(
            (s) => !BLOOD_STRUCTURES.has(s) && s !== Structure.None,
          ).length;
          if (endsOnTissue < 2)
            add(
              'edges',
              'warn',
              'Un extremo del caliper queda dentro de la cavidad, no sobre el borde endocárdico: la medida subestima el diámetro.',
            );
        ...
        if (inTarget < 0.6)
          add(
            'placement',
            'invalid',
            `El caliper no atraviesa ${pl.label} (${(inTarget * 100).toFixed(0)} % del segmento en la pared).`,
          );
        else if (inTarget < 0.85)
          add(
            'placement',
            'warn',
            `El caliper se sale de la pared (${(inTarget * 100).toFixed(0)} % del segmento en la pared): ajusta los extremos a los bordes.`,
          );
  ...
  if (spec.maxAngleDeg !== undefined && ctx.gate) {
    ...
    if (a === null || !ctx.gate.flowPresent)
      add(
        'alignment',
        'warn',
        'Sin flujo en el volumen de muestra en el instante de la medida: no se puede juzgar la alineación.',
      );
    else if (a <= spec.maxAngleDeg)
      add('alignment', 'ok', `Ángulo haz–flujo ${a.toFixed(0)}° (≤ ${spec.maxAngleDeg}°).`);
    else if (a <= spec.maxAngleDeg + 12)
      add(
        'alignment',
        'warn',
        `Ángulo haz–flujo ${a.toFixed(0)}°: la velocidad se subestima un ${((1 - Math.cos((a * Math.PI) / 180)) * 100).toFixed(0)} % (cos θ).`,
      );
    else
      add(
        'alignment',
  ...
  if (
    spec.tool === 'simpson' &&
    ctx.longAxisCm !== undefined &&
    ctx.trueLongAxisCm !== undefined &&
    ctx.trueLongAxisCm > 0
  ) {
    ...
    if (ratio < 0.82)
      add(
        'foreshortening',
        'invalid',
        `Ápex acortado: longitud trazada ${ctx.longAxisCm.toFixed(1)} cm frente a ${ctx.trueLongAxisCm.toFixed(1)} cm del modelo (${((1 - ratio) * 100).toFixed(0)} % menos); el volumen se subestima.`,
      );
    else if (ratio < 0.92)
      add(
        'foreshortening',
        'warn',
        `Ligero acortamiento apical (longitud ${ctx.longAxisCm.toFixed(1)} vs ${ctx.trueLongAxisCm.toFixed(1)} cm).`,
      );
  ...
  if (ctx.userAssisted)
    add(
      'assisted',
      'ok',
      'Envolvente trazada automáticamente; revisa que siga la señal más densa.',
    );
